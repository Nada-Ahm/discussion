...
router.post("/register",
    ...
    body("name").isString().withMessage("please enter a valid name!").isLength({ min: 3, max: 20 }).withMessage("name should be between (3-20)character"),
    body("password").isLength({ min: 8, max: 12 }).withMessage("password should be between (8-12)character"),
    body("phone").isLength({ min: 11, max: 20 }).withMessage("please enter a valid phone!"),
    async (req, res) => {
        try {
